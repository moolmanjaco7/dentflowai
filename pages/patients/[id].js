...
export default function PatientDetailPage() {
  ...
  return (
    <>
      <Head><title>Patient — DentFlowAI</title></Head>
      <main className="min-h-screen bg-slate-50">
        <section className="max-w-6xl mx-auto px-4 py-8">
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <div className="text-xs text-slate-500">
                <Link href="/patients" className="underline">Patients</Link> / Details
              </div>
              <h1 className="text-2xl font-bold text-slate-900 truncate">
                {patient?.full_name || "Patient"}
                {patient && (
                  <span className="ml-2 text-sm font-medium text-slate-500">
                    · {patient.patient_code || (patient.full_name ? baseFromName(patient.full_name) : "")}
                  </span>
                )}
              </h1>
            </div>
            <Link href="/patients">
              <button className="border rounded-md px-3 py-2 text-sm hover:bg-slate-50">Back</button>
            </Link>
          </div>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="grid lg:grid-cols-3 gap-4">
                ...
                <Card className="p-4">
                  <PatientQuickAdd patientId={patient.id} patientName={patient.full_name} />
                </Card>
              </div>
